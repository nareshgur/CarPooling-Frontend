...
import { MapPin, Calendar, Clock, Users, DollarSign, Car, Bike, Plus, FileText } from 'lucide-react';
...
export default function Publish() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const rideData = {
        vehicleId: 'mock-vehicle-id', // In real app, this would be selected from user's vehicles
        from: {
          id: `location-${Date.now()}-from`,
          ...form.from,
        },
        to: {
          id: `location-${Date.now()}-to`,
          ...form.to,
        },
        departureTime: new Date(`${form.departureDate}T${form.departureTime}`).toISOString(),
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="space-y-8">
        ...
        <div className="flex justify-between text-sm text-gray-500 mb-8">
          <span className={step >= 1 ? 'text-blue-600 font-medium' : ''}>Route</span>
          <span className={step >= 2 ? 'text-blue-600 font-medium' : ''}>Time</span>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    >
                      ...
                      <div className="text-center">
                        ...
                        <p className="text-sm text-gray-500">Comfortable and spacious</p>
                      </div>
                    </button>

                    <button
                      type="button"
                      onClick={() => setForm(prev => ({ ...prev, vehicleType: 'bike' }))}
                      className={`p-6 rounded-xl border-2 transition-all ${
                    ...
                    >
                      ...
                      <div className="text-center">
                        ...
                        <p className="text-sm text-gray-500">Fast and adventurous</p>
                      </div>
                    </button>
                  </div>
                </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <button
                        ...
                        className="p-2 rounded-lg border border-gray-300 hover:bg-gray-50"
                        disabled={form.availableSeats >= (form.vehicleType === 'bike' ? 1 : 8)}
                      >
                        +
                      </button>
                    </div>
                    <p className="text-sm text-gray-500 mt-1">
                      {form.vehicleType === 'bike' ? 'Maximum 1 passenger for motorcycles' : 'Maximum 8 passengers'}
                    </p>
                  </div>
            ...
            {step === 4 && (
              <div className="space-y-6">
                ...
                <div className="bg-gray-50 p-6 rounded-lg">
                  <h3 className="font-semibold text-gray-900 mb-4">Review Your Ride</h3>
                  
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <span className="text-gray-600">Vehicle:</span>
                      <div className="flex items-center space-x-2">
                        {form.vehicleType === 'car' ? <Car className="h-4 w-4" /> : <Bike className="h-4 w-4" />}
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span>{form.from.name} → {form.to.name}</span>
                    </div>
                    
                    <div className="flex items-center justify-between">
                      <span className="text-gray-600">Departure:</span>
                      <span>{form.departureDate} at {form.departureTime}</span>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-green-600 font-semibold">${form.pricePerSeat}</span>
                    </div>
                    ...
                    <div className="flex items-center justify-between font-semibold text-lg pt-2 border-t border-gray-200">
                      ...
                      <span className="text-green-600">${form.pricePerSeat * form.availableSeats}</span>
                    </div>
                  </div>
                </div>
              </div>
